...
Write::Write( DirList &dirlist, bool noDebugFlag, bool useGCC,
  ...
  writeHeader(noDebugFlag, useGCC, header_file);
  writeLists( dirlist );
  writeMainTargets( dirlist, programs_file );
  writeDependencies( dirlist );
  writeTrailer( dirlist, useGCC );
}

void Write::writeHeader(bool noDebugFlag, bool useGCC, string header_file) {
  ifstream in;
  time_t now( time( 0 ) );
...
void Write::writeLists( DirList &dirlist ){
...
void Write::writeMainTargets( DirList &dirlist, string programs_file ){
  ...
  if(in) { // read lines of "programs.mak"
    ...
    cout << "\n\n";
    writeMainTargetList( dirlist.cpp_main,
                            "$(CXX) $(CXXFLAGS)",
                            local_libs + " $(CCLIBFLAGS)" );
    writeMainTargetList( dirlist.c_main,
                            "$(CC) $(CFLAGS)" ,
                            local_libs + " $(CLIBFLAGS)");
    writeMainTargetList( dirlist.ass_main,
                            "$(CC) $(CFLAGS)" ,
                            local_libs + " $(CLIBFLAGS)");
  } 
}

void Write::writeMainTargetList( const set<string> &list,
                                    string compile,
                                    string local_libs ) {
...
void Write::writeDependencies( DirList &dirlist ){
  cout << "#\n"
       << "# Dependencies\n"
       << "#\n\n";

  writeDependencyList( dirlist.cpp, dirlist );
  writeDependencyList( dirlist.c, dirlist );
  cout << '\n';
}

void Write::writeDependencyList( const set<string> &list,
                                   DirList &dirlist ){
...
void Write::writeTrailer( DirList &dirlist, bool useGCC ){
  cout << "#\n";
  cout << "# Housekeeping\n";
  cout << "#\n";
  cout << "\n";
